function wind_steps()
    ...
    Windtyp                         = 4;  % 1: konstante Windgeschw., 2: IECwind  Windböe, 3: Turbulenter Wind, 4: Windgeschwindigkeitsstufen 
    Windgeschwindigkeit             = 10; % Konstante Windgeschw., wenn Windtyp = 1 ausgewählt wurde
    Durch_Windgeschwindigkeit       = 10; % Durchschnittliche Windgeschwindigkeit als Eingang für den Störungsbeobachter
    Boentyp                         = 6;  % 1: 6 m/s, 2: 8 m/s, 3: 10 m/s, 4: 11 m/s, 5: 12 m/s, 6: 14 m/s, 7: 16 m/s, 8: 18 m/s
    Turbolenter_Windtyp             = 5;  % Auswahl der mittleren Windgeschw. für den turbulenten Wind
    ...
    xlabel('Zeit t [s]','interpreter','latex')
    title('Windgeschwindigkeit vor der Rotorebene und Pitchwinkel')
    grid on

    % Ausgabe in PDF
    pos = get(gcf, 'Position');
    set(gcf, 'Position',pos+[-600 -450 600 450])
    filename = fullfile('./5. Validierung/3. Reglervalidierung/wind_steps.pdf');
